...
- The `Deploy website` job depends on `Verified`. It uses Vercel CLI 59.11.7 to pull project configuration, build the artifact and deploy with `--prebuilt`. The Hobby plan does not support native Git integration for private organization repositories, so Actions owns deployment.
